...
import { getLaunchableModels } from "../libs/LLM/index.ts";
import type {
  Command,
  ConfigQuantFile,
...
export default class LaunchCommand implements Command {
  ...
  async execute(argv: string[]): Promise<number> {
    ...
    const { values: argValues, positionals } = parseArgs({
      args: argv,
      allowPositionals: (this.constructor as typeof LaunchCommand)
        .allowPositionals,
      strict: true,
    ...
    switch (modelConfig.reasoningType) {
      case "reason_and_instruct":
        modelTypeConfigs =
          modelConfig.quantFiles?.[useReasoning ? "reasoning" : "instruct"];
        break;
      case "instruct_only":
    ...
    if (!modelConfig.quantFiles || !appConfig.QUANTIZATION_ORDER) {
      throw createError(
        simpleTemplate(appState.s.e.c.lu.undefinedLauncher, {
      ...
      try {
        ...
        if (isEnoentError(err)) {
          throw createError(
            ...
            { code: "QUANT_FILE_NOT_FOUND", cause: err },
          );
        }
        throw err;
      }
    ...
    const commandArgs: string[] = [
      ...normalizeArgs(appConfig.LLAMACPP_BASE_ARGS),
    ];
    ...
    if (useMultimodal) {
      if (!mmprojPath) {
        throw createError(
          simpleTemplate(appState.s.e.c.lu.mmprojNotConfigured, {
            ModelPreset: modelKey,
          }),
      ...
      try {
        ...
        if (isEnoentError(err)) {
          throw createError(
            ...
            { code: "MMPROJ_FILE_NOT_FOUND", cause: err },
          );
        }
        throw err;
      }
      ...
      log(
        simpleTemplate(appState.s.m.c.lu.found, {
    ...
    try {
      if (fullCommandArray.length === 0) {
        throw createError(appState.s.e.c.lu.emptyCMD, {
          code: "EMPTY_CMD",
        });
      }
      const cmd = fullCommandArray[0]!;
      const args = fullCommandArray.slice(1);
      ...
      if (code !== 0 && code !== null) {
        errlog(
          simpleTemplate(appState.s.m.c.lu.llamaServerExit, { ExitCode: code }),
        );
      }
      return code ?? 0;
    } catch (err) {
      if (isNodeError(err)) {
        createError(
          red(
            simpleTemplate(appState.s.e.c.lu.failedToStart, {
              Error: err.message,
            }),
          ),
          {

...
function createOpenAILine(
  ...
  const body: { model: string; messages: Message[]; temperature?: number; reasoning?: { effort: string } } = {
    model,
    messages,
  };
...
export default class BatchGenCommand implements Command {
  ...
  async execute(argv: string[]): Promise<number> {
    ...
    if (await target.exists()) {
      throw createError(
        simpleTemplate(appState.s.e.lllm.targetFileExists, { TargetPath: targetPath }),
        { code: "TARGET_EXISTS" },
      );
    }
      ...
      for (const [index, chunk] of textChunks.entries()) {
        ...
        switch (format) {
          case "openai": {
            ...
            jsonlLine = createOpenAILine(requestId, modelName, messages, temperature, finalReasoningObject);
            break;
          }
          case "gemini": {

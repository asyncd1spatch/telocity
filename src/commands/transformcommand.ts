...
import { buildImageContent, dummy, getPresetHelpText, UILLMIO } from "../libs/LLM";
...
export default class TransformCommand implements Command {
  ...
  async execute(argv: string[]): Promise<number> {
    ...
    const { values: argValues, positionals } = parseArgs({
      ...
      allowPositionals: (this.constructor as typeof TransformCommand).allowPositionals,
      strict: true,
    });
    ...
    if (presetChatTemplateKwargs?.[0] && presetChatTemplateKwargs[1]?.["reasoning_effort"]) {
      let reasoningEffort: "low" | "medium" | "high";
      const budgetMap: Record<string, "low" | "medium" | "high"> = { "1": "low", "2": "medium", "3": "high" };
      const budgetValue = argValues.budget as string | undefined;

      if (useReasoning) {
        if (budgetValue && budgetMap[budgetValue]) {
          reasoningEffort = budgetMap[budgetValue];
        } else {
          const presetEffort = presetChatTemplateKwargs[1]["reasoning_effort"] as "low" | "medium" | "high";
    ...
    if (argValues.prompt) {
      ...
      prependPromptFinal = [true, promptSettings.defPrep[1] + "\n\n", roletag2, true];
    } else {
      prependPromptFinal = [true, appConfig.DEFAULT_TF_PROMPT + "\n\n", roletag2, false];
    }

  ...
  static get MODELS_TO_DOWNLOAD(): Readonly<{
    ...
    return {
      ...
      qwen: [
        ...
        "https://huggingface.co/Qwen/Qwen3-4B/resolve/main/tokenizer_config.json",
      ],
      gptoss: [
      ...
      try {
        ...
        const tasks = [
          async () => {
            ...
            const timeout = setTimeout(() => controller.abort(), HARD_TIMEOUT);

            try {
              return await fetch(modelUrl!, { signal: controller.signal });
            } finally {

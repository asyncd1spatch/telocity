...
export default class OneShotCommand implements Command {
  ...
  async execute(argv: string[]): Promise<number> {
    ...
    if (promptSettings.defPrefill?.[0]) {
      prefillPromptFinal = promptSettings.defPrefill;
    }
    ...
    const options: LLMConfigurableProps = {
      ...
      chunkSize: Number(OneShotCommand.defaultChunkSize),
      batchSize: Number(OneShotCommand.defaultBatchSize),
      parallel: Number(OneShotCommand.defaultParallel),

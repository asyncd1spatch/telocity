...
export class StreamedLineWrapper {
  ...
  private lastSegment = { text: "", width: 0, isWhitespace: false };
  ...
  private async flushOutput() {
    if (!this.outputBufferChunks.length) return;
  ...
  private appendSegment(segment: string) {
    ...
    if (this.currentLineWidth === 0 && isWhitespace) {
      return;
    }
    ...
    if (!isWhitespace && segmentWidth > this.terminalWidth) {
      ...
      for (const { segment: grapheme } of this.graphemeSegmenter.segment(segment)) {
      ...
      this.lastSegment = { text: "...", width: this.currentLineWidth, isWhitespace: false };
      return;
    }

    if (this.currentLineWidth > 0 && this.currentLineWidth + segmentWidth > this.terminalWidth) {
      if (this.lastSegment.isWhitespace) {
        this.outputBufferChunks.pop();
...
export function generateHelpText(
  ...
  if ("commandDescriptions" in helpSection && "commandHeader" in helpSection) {
    ...
    if (footerText) {
      lines.push("", ...wrapText(footerText, { width: appState.TERMINAL_WIDTH }));
    }
    ...
    lines.push(...wrapText(usageText, { width: appState.TERMINAL_WIDTH }));
    lines.push("", ...wrapText(descriptionText, { width: appState.TERMINAL_WIDTH }));

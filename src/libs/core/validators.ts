...
export const V = {
  ...
  delay:
    ...
    (val: unknown) => {
      ...
      if (Number.isNaN(numericValue)) {
        throw createError(
          errorMsg.replace(errorPlaceholder, String(numericValue)),
          { code: errorCode },
        );
      }
      ...
      if (
        constraints.minExclusive !== undefined &&
        numericValue <= constraints.minExclusive
      ) {
        throw createError(
          errorMsg.replace(errorPlaceholder, String(numericValue)),
          { code: errorCode },
      ...
      if (
        constraints.maxExclusive !== undefined &&
        numericValue >= constraints.maxExclusive
      ) {
        throw createError(
          errorMsg.replace(errorPlaceholder, String(numericValue)),
          { code: errorCode },
  ...
  getDelaySeconds:
    (errorMsg: string, errorCode: string, errorPlaceholder = "{{ .Value }}") =>
    (value: unknown): DelayTuple => {
      ...
      if (
        ...
        [isProcessed, numericValue] = value as [boolean, unknown];
      } else {

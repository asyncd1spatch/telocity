...
export class LLM {
  ...
  protected readonly llmbackend: backend = "responses";
  protected readonly url: string = "http://localhost:8080/v1/responses";
  ...
  private async makeRequest(
    payload: OpenAIPayload | ResponsesPayload,
  ...
  private buildPayload(
    messages: Message[],
    stream: boolean,
  ): OpenAIPayload | ResponsesPayload {
    ...
    if (this.llmbackend === "chatcompletions") {
      ...
      const inputMessages: ResponsesMessage[] = messages.reduce((acc, msg) => {
        if (msg.role === "system") {
          const content =
          ...
          if (typeof msg.content === "string") {
            newContent.push({ type: "input_text", text: msg.content });
          } else {
            for (const part of msg.content) {
              if (part.type === "text") {
                newContent.push({ type: "input_text", text: part.text });
          ...
          acc.push({ role: msg.role, content: newContent });
        }
        return acc;
      }, [] as ResponsesMessage[]);
      ...
      try {
        ...
        while (true) {
          ...
          while ((eventEndIndex = buffer.indexOf("\n\n")) >= 0) {
            ...
            try {
              const parsed = JSON.parse(eventData) as SSEChunk;
  ...
  protected async *chatCompletionsStreamResponses(
    messages: Message[],
    { idleTimeout = false } = {},
  ): AsyncGenerator<string, void, unknown> {
  ...
  protected async *chatCompletionsStreamLegacy(
    messages: Message[],
    { idleTimeout = false } = {},
  ): AsyncGenerator<string, void, unknown> {
    const controller = new AbortController();
    ...
    for await (const chunk of this.readSSEAndParse(
      payload,
      controller,
      this.parseLegacyChatChunk.bind(this),
      { idleTimeout },
    )) {
      yield chunk;
    }
  ...
  protected async *chatCompletionsStream(
    ...
    if (this.llmbackend === "responses") {
      for await (const c of this.chatCompletionsStreamResponses(messages, {
        idleTimeout,
      })) {
        yield c;
      }
    } else {
      for await (const c of this.chatCompletionsStreamLegacy(messages, {
        idleTimeout,
      })) {
        yield c;
      }
  ...
  protected async chatCompletionsNoStream(
    messages: Message[],
  ): Promise<string> {
    const controller = new AbortController();
    let timeoutId: ReturnType<typeof setTimeout> | null = null;
  ...
  protected async chatCompletions(
    ...
    if (!verbose && isBun) {
      return await this.chatCompletionsNoStream(messages);
    }

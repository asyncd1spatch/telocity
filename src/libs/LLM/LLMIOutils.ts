...
export async function splitFile(
  ...
  const flushBuffer = async () => {
    ...
    const canWrite = currentWriter.write(data);
    if (!canWrite) {
      await new Promise<void>((resolve) =>
        currentWriter!.once("drain", resolve),
      );
    }
    writeBuffer = [];
    bufferedBytes = 0;
  ...
  const createNewPart = async () => {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      currentWriter!.once("error", (err) => reject(err));
    });
...
export async function mergeFiles(
  ...
  try {
    ...
    for (const { filePath } of files) {
      ...
      const headerBuf = Buffer.from(fileHeaderStr, "utf8");
      writeBuffer.push(headerBuf);
      bufferedBytes += headerBuf.length;

      let readStream: ReadStream | null = null;
      try {
        readStream = createReadStream(filePath);
        for await (const chunk of readStream) {
          const bufChunk = chunk as Uint8Array;
          writeBuffer.push(bufChunk);
          bufferedBytes += bufChunk.length;

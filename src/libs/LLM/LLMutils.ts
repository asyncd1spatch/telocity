...
export async function dummy(
  ...
  function* dummyStream(): Generator<string, void, unknown> {
    ...
    for (const param of OPT_PARAMS) {
      if (payload[param as keyof ChatCompletionsPayload] !== undefined) {
        const value = payload[param as keyof ChatCompletionsPayload];
...
export function getDefaultModelParam(
  param: "chunkSize" | "batchSize" | "parallel",
): string {
  ...
  if (modelConfig.reasoningType === "reason_and_instruct") {
    activeVariant = modelConfig.instruct;
  } else {
    activeVariant = modelConfig.default;
  }

  const val = activeVariant.model[param];

  return val !== undefined ? String(val) : appConfig.FALLBACK_VALUES[param];
}

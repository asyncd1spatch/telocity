...
export class LLMIO extends LLM {
  ...
  private static async loadProgressState(hash: string): Promise<ProgressState | null> {
  ...
  async processBatch() {
    ...
    const tasks = batch.map(chunk => () => this.completion(this.newPrompt(chunk)));

...
export class LLMBATCHER extends LLM {
  ...
  async processBatch() {
    ...
    const tasks = batch.map((chunk) => async () => {
      const MAX_ATTEMPTS = 4;
      const TEMP_INCREMENT = 0.3;
      const MIN_RETRY_START_TEMP = 0.4;
      const MAX_TEMP_CAP = 1.0;

      const configuredDelay = this.delay[1] ?? 60000;

      let currentTemp =
        this.temperature && this.temperature[0] ? this.temperature[1] : 0.7;

      for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        try {
          return await this.completion(this.newPrompt(chunk), {
            overrides: {
              temperature: [true, currentTemp],
            },
          });
        } catch (err) {
          ...
          let nextTemp = currentTemp + TEMP_INCREMENT;

          if (nextTemp < MIN_RETRY_START_TEMP) {
            nextTemp = MIN_RETRY_START_TEMP;
          }

          currentTemp = Number(Math.min(nextTemp, MAX_TEMP_CAP).toFixed(1));
          ...
          const retryWait = Math.max(configuredDelay, attempt * 1000);
          await new Promise((resolve) => setTimeout(resolve, retryWait));
        }
      }
      throw createError(this.appState.s.e.lllm.unreachable, {
        code: "INTERNAL_ERROR",
      });
    });

    return await runConcur(tasks, { concurrency: this.parallel });
  }

...
export class LLMBATCHER extends LLM {
      ...
      .map((chunk) => {
        const run = async (
          ...
          try {
            ...
            const nextTemp = Math.min(1.0, +(temp + 0.3).toFixed(1));

            const configuredDelayMs = this.delay[0] ? this.delay[1] * 1000 : 0;
            const waitTime = Math.max(configuredDelayMs, attempt * 1000);
  ...
  async *generateProcessedBatches() {
    ...
    while (this.lastIndex < this.chunks.length) {
      ...
      const configuredDelayMs = this.delay[0] ? this.delay[1] * 1000 : 0;
      const delay = Math.max(configuredDelayMs - elapsed, 0);

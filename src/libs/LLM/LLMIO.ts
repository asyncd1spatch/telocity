...
function getArgConfig() {
  ...
  _ARG_CONFIG = {
    chunkSize: {
      prop: "chunkSize" as keyof (LLMBATCHER & LLMConfigurableProps),
      validate: V.num(
    ...
    batchSize: {
      prop: "batchSize" as keyof (LLMBATCHER & LLMConfigurableProps),
      validate: V.num(
    ...
    parallel: {
      prop: "parallel" as keyof (LLMBATCHER & LLMConfigurableProps),
      validate: V.num(
        { minExclusive: 0, max: 64, integer: true, allowNaN: false },
        appState.s.e.v.invalidBatchSize,
        "INVALID_BATCH_SIZE",
...
export class LLMBATCHER extends LLM {
  ...
  async processBatch() {
    ...
    const result = await runConcur(tasks, { concurrency: this.parallel });

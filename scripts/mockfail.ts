...
async function requestHandler(req: IncomingMessage, res: ServerResponse) {
  ...
  if (SHOULD_TIMEOUT_MODE) {
    if (attempts < 3) {
      console.log(
        `\x1b[33m[MOCK] TIMEOUT MODE: Delaying 5s (Attempt ${attempts + 1}/2)\x1b[0m`,
      );
    ...
    if (attempts < 3) {
      failureTracker.set(promptKey, attempts + 1);
      console.log(
        `\x1b[31m[MOCK] STATUS: FAILED (Attempt ${attempts + 1}/2)\x1b[0m`,
      );
  ...
  console.log(`\x1b[32m[MOCK] STATUS: SUCCESS | Temp: ${currentTemp}\x1b[0m`);
  ...
  if (isStreaming) {
    ...
    const sseData = { choices: [{ delta: { content: responseText } }] };
...
server.listen(MOCK_PORT, () => {
  ...
  if (SHOULD_TIMEOUT_MODE) {
    ...
    console.log(`  BEHAVIOR: 3 Delays (5s) -> 1 Success (No delay)`);
  } else {
    console.log(`  MODE: \x1b[34mSTANDARD ERROR SIMULATION\x1b[0m`);
    console.log(`  BEHAVIOR: 2 Failures (500) -> 1 Success`);
  }
  console.log(
    "================================================================",
  );

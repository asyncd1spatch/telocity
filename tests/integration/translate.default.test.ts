...
import { AppStateSingleton, config as appConfig } from "../../src/libs/core";
import type { ConfigModelVariant } from "../../src/libs/types";
...
switch (modelConfig.reasoningType) {
  ...
  default:
    ...
    throw new Error(`Unhandled reasoning type in test setup: ${(_exhaustiveCheck as any)?.reasoningType}`);
}
...
describe("Translate Command (Default Args)", () => {
  ...
  test("handles default arguments correctly", async () => {
    ...
    [
      "Translate any text you're given",

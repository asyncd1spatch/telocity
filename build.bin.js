...
for (const buildTarget of targets) {
  ...
  const exeFileName = target.startsWith("bun-windows") ? "telocity.exe" : "telocity";
  ...
  await Bun.build({
    entrypoints: ["./start.ts", "tokenworker.ts"],
    outdir: "./dist",
    format: "esm",
    minify: true,
    define: {
  ...
  if (!fs.existsSync(generatedExePath)) {
    console.warn(`Warning: expected output file not found: ${generatedExePath}`);
  } else {
  ...
  if (shouldCompress) {
    if (!fs.existsSync(generatedExePath)) {
      console.warn(`Skipping compression for ${os} because the expected binary wasn't found.`);
      continue;
    }
    ...
    console.log(`Compressing ${generatedExePath} -> ${zipPath} using 7z.exe (max compression)...`);
    try {
      const result = await $`7z a -t7z -m0=lzma2 -mx=9 -mfb=273 -md=256m -ms=on ${zipPath}.7z ${generatedExePath}`;
      if (result && (result.exitCode === 0 || result.success === true)) {
      ...
      console.error("Compression step failed. Make sure 7z is installed and on PATH.", err);
      throw err;
    }
  } else {
